...
prompt_template: ChatPromptTemplate = ChatPromptTemplate.from_messages([
    ('system', 'You are a helpful AI Assistant specializing in agentic AI system'),
    MessagesPlaceholder(variable_name='history'),  # expands to all past returns
    ('human', '{question}')])  # current user input
...
# ---------------------------------------------------------------------------
# Short-term history for the current session (same as main_03)
# ---------------------------------------------------------------------------
history: list[BaseMessage] = []

print(f"\n=== Loop + Short-Term + Long-Term Memory ===")
...
try:
    while True:
        ...
        print(f"  [sending {len(history) + 2} messages to LLM]")
