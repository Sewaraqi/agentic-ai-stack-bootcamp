...
class RagConfig:
    top_k: int = 4
    refuse_threshold: float = 0.3   # reject answers when best chunk score is below this
    use_mmr: bool = False
...
class RagPipeline:
    ...
    def answer_with_sources(self, question: str) -> tuple[str, list[RetrievalResult]]:
        ...
        if self._should_refuse(results):
            return "I don't have reliable information on that topic in my knowledge base.", []
        ...
        messages = [
            SystemMessage(content="Answer using ONLY the provided context. Cite the source filename at the end."),
